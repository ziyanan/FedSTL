...
def generate_property_test(tensor_arr, property_type = "upper", mining_range = 2):
    ...
    sliding_window = torch.zeros(tensor_chunked.shape[3], 3).to(device)     # hrs * (mean, lowerbound, upperbound)
    property = torch.zeros([tensor_chunked.shape[0], tensor_chunked.shape[3]]).to(device)
    ...
    for j in range(tensor_chunked.shape[3]):
        ...
        sliding_window[j,1] = torch.min(tensor_chunked[:,0,:,j])
        sliding_window[j,2] = torch.max(tensor_chunked[:,0,:,j])
...
def generate_property(tensor_arr, property_type = "corr", mining_range = 2):
    optmethod = "gradient"
    conf_interval       = torch.zeros(tensor_arr.shape[1], 3).to(device)        # batch * 1 * hrs * (mean, lowerbound, upperbound)
    conf_interval_guide = torch.zeros(tensor_arr.shape[1], 3).to(device)        # batch * 1 * hrs * (mean, lowerbound, upperbound)
    sliding_window      = torch.zeros(tensor_arr.shape[1], 3).to(device)        # hrs * (mean, lowerbound, upperbound)
    property            = torch.zeros([tensor_arr.shape[0], tensor_arr.shape[1]]).to(device)
    
    for j in range(tensor_arr.shape[1]):
        mean, conf_low, conf_up = torch_confidence_interval(tensor_arr[:,j,0])
        conf_interval[j,0]  = mean
        conf_interval[j,1]  = conf_low
        conf_interval[j,2]  = conf_up
        sliding_window[j,1] = torch.min(tensor_arr[:,j,0])
        sliding_window[j,2] = torch.max(tensor_arr[:,j,0])

        mean_guide, conf_low_guide, conf_up_guide = torch_confidence_interval(tensor_arr[:,j,1])
        conf_interval_guide[j,0] = mean_guide
        conf_interval_guide[j,1] = conf_low_guide
        conf_interval_guide[j,2] = conf_up_guide
...
def get_robustness_score(tensor_arr, pred, property_type = "upper", mining_range = 2):
    ...
    # # tensor_arr = np.column_stack((tensor_arr[:,1:], tensor_arr[:,0].reshape(-1,1))) # shift data
    
    # conf_interval = torch.zeros([tensor_arr.shape[1], 3]).to(device)   # hrs * (mean, lowerbound, upperbound)
    # sliding_window = torch.zeros([tensor_arr.shape[1], 3]).to(device)   # hrs * (mean, lowerbound, upperbound)
    # property = torch.zeros([tensor_arr.shape[1]]).to(device)

    # # idx = np.random.randint(tensor_arr.shape[0], size=int(tensor_arr.shape[0]*0.2))
    # # tensor_arr = tensor_arr[idx,:]

    # for j in range(conf_interval.shape[0]):
    #     mean, conf_low, conf_up = mean_confidence_interval(tensor_arr[:,j])
    #     conf_interval[j,0] = mean
    #     conf_interval[j,1] = conf_low
    #     conf_interval[j,2] = conf_up
    #     sliding_window[j,1] = min(tensor_arr[:,j])
    #     sliding_window[j,2] = max(tensor_arr[:,j])

    # if property_type == "mean":
    #     for temp_idx, templ in enumerate(STL_templates[property_type]):
    #         try:
    #             (stlsyn, value, dur) = synth.synthSTLParam(templ, sliding_window[:,0], optmethod)
    #             property[mining_range*temp_idx:mining_range*temp_idx+mining_range] = stlsyn.subformula.bound
    #         except:
    #             property[mining_range*temp_idx:mining_range*temp_idx+mining_range] = sliding_window[mining_range*temp_idx:mining_range*temp_idx+mining_range, 0]

    # elif property_type == "upper":
    #     for temp_idx, templ in enumerate(STL_templates[property_type]):
    #         try:
    #             (stlsyn, value, dur) = synth.synthSTLParam(templ, sliding_window[:,2], optmethod)
    #             property[mining_range*temp_idx:mining_range*temp_idx+mining_range] = stlsyn.subformula.bound
    #         except:
    #             property[mining_range*temp_idx:mining_range*temp_idx+mining_range] = sliding_window[mining_range*temp_idx:mining_range*temp_idx+mining_range, 2]
    ...
    #     for temp_idx, templ in enumerate(STL_templates[property_type]):
    #         try:
    #             (stlsyn, value, dur) = synth.synthSTLParam(templ, sliding_window[:,1], optmethod)
    #             property[mining_range*temp_idx:mining_range*temp_idx+mining_range] = stlsyn.subformula.bound
    #         except:
    #             property[mining_range*temp_idx:mining_range*temp_idx+mining_range] = sliding_window[mining_range*temp_idx:mining_range*temp_idx+mining_range, 1]
                    
    
    # elif property_type == "delta":
    #     raise NotImplementedError
    
    # return conf_interval, property